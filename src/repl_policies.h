...
class OptBypassPolicy : public OptReplPolicy {
    ...
    public:
        explicit OptBypassPolicy(uint32_t _numLines, const char* countFile) : OptReplPolicy(_numLines, countFile) {}
        
        bool needBypassWithCands(uint64_t addr, const MemReq* req, SetAssocCands cands) override {
            if(req->replaced_cache_line_addr && *(req->replaced_cache_line_addr)) return false; // second access generated by bypass
            
            uint64_t bestScore = 0;
            for (auto ci = cands.begin(); ci != cands.end(); ci.inc()) {
                auto id = *ci;
                if(!cc->isValid(id)) // if there is invalid block, do not bypass
                    return false;
                ...
                if(future_counts[array[id]].empty()) // if there is block not be used in future, do not bypass
                    return false;
                auto s = future_counts[array[id]].back();
                if(s>bestScore)
                    bestScore = s;
            ...
            // printf("opt bypass check final\n");
            // printf("bestScore = %lu\n", bestScore);
            // if(!future_counts[addr].empty())
            //     printf("request score = %lu\n", future_counts[addr].back());
